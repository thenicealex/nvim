...
local fn = vim.fn
local tabufline_config = require("tabufline.config")
...
local function styleBufferTab(nr)
	...
	if tabufline_config.colored then
		name = "%#TbLineBufOnClose#" .. name .. "%#TbLineBufOnClose#"
	end
...
M.NvimTreeOverlay = function()
	...
	return "%#NvimTreeNormal#" .. (flag==false and "" or string.rep(" ", width+1))
end
